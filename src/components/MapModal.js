...
export function MapModalProvider({ children }) {
  ...
  const openMap = useCallback((name, lat, lng) => {
    setState({ open: true, name: name, lat: lat, lng: lng });
  ...
  return (
    <MapModalContext.Provider value={{ openMap, closeMap }}>
      {children}
      <MapModal state={state} closeMap={closeMap} />
    </MapModalContext.Provider>
  );
}

/* ── OSM Tile renderer (ported from renderTileMap) ── */
function renderTileMap(canvas, loader, lat, lng, zoom) {
  ...
  function latLngToPixel(lat, lng, z, cLat, cLng, W, H) {
    ...
    var cx = toX(cLng),
      cy = toY(cLat);
    var px = toX(lng) - cx + W / 2;
    var py = toY(lat) - cy + H / 2;
    return { x: px, y: py };
  }
  ...
  for (var ix = 0; ix < tilesX; ix++) {
    for (var iy = 0; iy < tilesY; iy++) {
      (function (ix, iy) {
        ...
        img.onload = function () {
          ctx.drawImage(img, drawX0 + ix * 256, drawY0 + iy * 256, 256, 256);
          loaded++;
          done();
        };
...
function MapModal({ state, closeMap }) {
